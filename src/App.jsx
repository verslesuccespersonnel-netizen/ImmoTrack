  ...
  return (
    <div style={{ minHeight:'100vh', background:'#F7F5F0', display:'flex', alignItems:'center', justifyContent:'center', padding:24, fontFamily:'sans-serif' }}>
      <div style={{ background:'#fff', borderRadius:16, border:'1px solid rgba(0,0,0,0.08)', padding:'36px 32px', maxWidth:520, width:'100%' }}>
        ...
        <h2 style={{ fontFamily:'Georgia,serif', fontSize:20, marginBottom:10 }}>🔑 Configuration requise</h2>
        <div style={{ background:'#1A1714', borderRadius:8, padding:'14px 16px', marginBottom:12 }}>
          <div style={{ fontFamily:'monospace', fontSize:12, color:'#E8F2EB', lineHeight:2 }}>
            ...
            <div><span style={{ color:'#C8813A' }}>REACT_APP_SUPABASE_URL</span><span style={{ color:'#9E9890' }}> = </span><span style={{ color:'#7EB89A' }}>https://xxxx.supabase.co</span></div>
            <div><span style={{ color:'#C8813A' }}>REACT_APP_SUPABASE_ANON_KEY</span><span style={{ color:'#9E9890' }}> = </span><span style={{ color:'#7EB89A' }}>eyJhbGciOiJIUzI1NiIs...</span></div>
          </div>
        </div>
        <div style={{ background:'#E8F2EB', borderRadius:8, padding:'12px 14px', fontSize:13, color:'#2D5A3D' }}>
          Après ajout : <strong>Vercel → Deployments → ⋯ → Redeploy</strong>
        </div>
      </div>
    </div>
  )
...
function ProtectedRoute({ children, roles }) {
  ...
  if (loading) return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', minHeight:'100vh', flexDirection:'column', gap:16, background:'#F7F5F0', fontFamily:'sans-serif' }}>
      <div style={{ fontFamily:'Georgia,serif', fontSize:22 }}><span style={{ color:'#2D5A3D' }}>Immo</span><span style={{ color:'#C8813A' }}>Track</span></div>
