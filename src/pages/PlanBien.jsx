...
const ECAT={
  ...
  'Électricité':[{n:'Prise',i:'🔌'},{n:'Interrupteur',i:'🔘'},{n:'Tableau élec.',i:'⚡'},{n:'Lumière',i:'💡'},{n:'VMC',i:'💨'},{n:'Sonnette',i:'🔔'},{n:'Clim',i:'❄️'},{n:'Détect. fumée',i:'🔴'}],
...
// État d'interaction stocké dans un ref (jamais de re-render pendant le drag)
// mode: null | 'drag' | 'drag_e' | 'resize'
const mkInter = () => ({ mode:null, id:null, ox:0, oy:0, sx:0, sy:0, sw:0, sh:0 })
...
export default function PlanBien() {
  ...
  // ── Gestionnaires window mousemove/mouseup ────────────────
  // Attachés une seule fois, lisent toujours le state via les refs
  useEffect(() => {
    function onMove(e) {
      ...
      if (it.mode === 'drag') {
        ...
        if (!p) return
        const nx = Math.max(0, Math.min(GW - p.w, col - it.ox))
        const ny = Math.max(0, Math.min(GH - p.h, row - it.oy))
        setPlan(prev => prev.map(x => x.id === it.id ? {...x, x:nx, y:ny} : x))
      }
      ...
      if (it.mode === 'resize') {
        ...
        const p  = planRef.current.find(x => x.id === it.id)
        if (!p) return
        const nw = Math.max(1, Math.min(GW - p.x, it.sw + dw))
        const nh = Math.max(1, Math.min(GH - p.y, it.sh + dh))
        setPlan(prev => prev.map(x => x.id === it.id ? {...x, w:nw, h:nh} : x))
      }
    }

    async function onUp() {
      const it = inter.current
      if (!it.mode) { inter.current = mkInter(); return }
  ...
  function onPieceDown(e, p) {
    // Sélectionner + préparer drag — ne PAS setPointerCapture
    e.stopPropagation()
    setSelP(p.id); setSelE(null)
    const { col, row } = getOff(e)
    inter.current = { mode:'drag', id:p.id, ox:col-p.x, oy:row-p.y, sw:p.w, sh:p.h, sx:0, sy:0 }
  }

  function onEquipDown(e, eq) {
    e.stopPropagation()
    setSelE(eq.id); setSelP(null)
    inter.current = { mode:'drag_e', id:eq.id, sx:e.clientX, sy:e.clientY, ox:eq.grid_x||0, oy:eq.grid_y||0, sw:0, sh:0 }
  }

  function onResizeDown(e, p) {
    // CRUCIAL : stopPropagation empêche le handler du parent (drag) de se déclencher
    e.stopPropagation()
    e.preventDefault()
    inter.current = { mode:'resize', id:p.id, sx:e.clientX, sy:e.clientY, sw:p.w, sh:p.h, ox:0, oy:0 }
  }
    ...
    <Layout>
      ...
      <div style={{display:'flex',gap:10,height:'calc(100vh - 195px)',minHeight:450}}>
        ...
        <div style={{width:155,background:'#fff',border:'1px solid rgba(0,0,0,.08)',borderRadius:12,padding:8,overflowY:'auto',flexShrink:0}}>
          ...
          {tab==='equips'&&<>
            <div style={{fontSize:9,fontWeight:700,color:'#9E9890',textTransform:'uppercase',marginBottom:4}}>{selPiece?<span>→ <span style={{color:'#2D5A3D'}}>{selPiece.nom}</span></span>:'Sélect. pièce d\'abord'}</div>
        ...
        <div style={{flex:1,overflow:'auto',background:'#F0EDE6',borderRadius:12,border:'1px solid rgba(0,0,0,.08)'}}>
          <svg ref={svgRef} width={GW*CELL} height={GH*CELL}
            ...
            onClick={e=>{if(inter.current.mode) return; setSelP(null); setSelE(null)}}>
            ...
            {plan.map(p=>{
              ...
              return (
                <g key={p.id}>
                  {/* Zone de drag — occupe toute la pièce SAUF le coin resize */}
                  <rect
                    ...
                    style={{cursor:'grab'}}
                    onMouseDown={e=>onPieceDown(e,p)}
                    onDoubleClick={e=>{e.stopPropagation();if(inter.current.mode)return;setForm({nom:p.nom,icone:p.icone,couleur:p.couleur,w:p.w,h:p.h});setModal({type:'piece',id:p.id})}}
                  />
                  ...
                  {/* Bouton supprimer — seulement quand sélectionné */}
                  {isSel&&(
                    <g onClick={e=>{e.stopPropagation();if(inter.current.mode)return;delPiece(p.id)}} style={{cursor:'pointer'}}>
                      <circle cx={px+pw-11} cy={py+11} r="10" fill="#B83232" opacity=".9"/>
                      <text x={px+pw-11} y={py+11} textAnchor="middle" dominantBaseline="middle" fontSize="13" fill="white" style={{pointerEvents:'none'}}>✕</text>
                    </g>
                  )}

                  {/* ── POIGNÉE RESIZE ──
                      Placée DANS le SVG du plan, handler onMouseDown avec stopPropagation.
                      Le window.onmousemove/mouseup gère ensuite le mouvement.
                      Toujours affichée sur la pièce sélectionnée.
                  */}
                  {isSel&&(
                    <g style={{cursor:'se-resize'}}
                      onMouseDown={e=>onResizeDown(e,p)}
                      onClick={e=>e.stopPropagation()}>
                      {/* Grand fond vert pour faciliter le clic */}
                      <rect x={px+pw-22} y={py+ph-22} width="20" height="20" rx="5"
                        fill="#2D5A3D" opacity=".9"/>
                      <text x={px+pw-12} y={py+ph-12} textAnchor="middle" dominantBaseline="middle"
                        fontSize="12" fill="white" style={{pointerEvents:'none',userSelect:'none'}}>↘</text>
                    </g>
                  )}
                  {/* Petit indicateur discret sur pièces non sélectionnées */}
                  {!isSel&&(
                    <rect x={px+pw-7} y={py+ph-7} width="5" height="5" rx="1"
                      fill="rgba(0,0,0,.15)" style={{pointerEvents:'none'}}/>
                  )}
                </g>
              )
            })}
            ...
            {equips.map(eq=>{
              ...
              return (
                <g key={eq.id} style={{cursor:'grab'}}
                  onMouseDown={e=>onEquipDown(e,eq)}
                  onDoubleClick={e=>{e.stopPropagation();if(inter.current.mode)return;setForm({nom:eq.nom,icone:eq.icone,notes:eq.notes||'',piece_id:eq.plan_piece_id||''});setModal({type:'equip',id:eq.id})}}>
                  ...
                  {isSel&&<>
                    ...
                    <g onClick={e=>{e.stopPropagation();if(inter.current.mode)return;delEquip(eq.id)}} style={{cursor:'pointer'}}>
                      <circle cx={ex+22} cy={ey+2} r="8" fill="#B83232" opacity=".9"/>
                      <text x={ex+22} y={ey+2} textAnchor="middle" dominantBaseline="middle" fontSize="10" fill="white" style={{pointerEvents:'none'}}>✕</text>
                    </g>
                  </>}
                </g>
              )
            ...
            {plan.length===0&&equips.length===0&&(
              <text x={GW*CELL/2} y={GH*CELL/2} textAnchor="middle" fontSize="14" fill="rgba(0,0,0,.2)" fontFamily="sans-serif">← Cliquez sur une pièce dans la palette pour commencer</text>
            )}
          </svg>
        </div>
        ...
        <div style={{width:155,background:'#fff',border:'1px solid rgba(0,0,0,.08)',borderRadius:12,padding:12,overflowY:'auto',flexShrink:0}}>
          {selPiece&&<>
            ...
            <div style={{fontSize:11,color:'#6B6560',marginBottom:10}}>{selPiece.w}×{selPiece.h} cellules</div>
          ...
          {!selPiece&&!selEquip&&<>
            ...
            {plan.map(p=>(
              <div key={p.id} onClick={()=>setSelP(p.id)} style={{display:'flex',alignItems:'center',gap:5,padding:'3px 4px',borderRadius:5,cursor:'pointer',background:selP===p.id?'#E8F2EB':'transparent',marginBottom:2}}>
              ...
              {modal.type==='piece'&&<>
                ...
                <div className="fld"><label>Couleur</label>
                  <div style={{display:'flex',gap:5,flexWrap:'wrap',marginTop:4}}>
                    {PALCOLS.map(c=><div key={c} onClick={()=>set('couleur',c)} style={{width:24,height:24,borderRadius:5,background:c,cursor:'pointer',border:`2.5px solid ${form.couleur===c?'#2D5A3D':'rgba(0,0,0,.12)'}`}}/>)}
                  </div>
                </div>
              </>}
              {modal.type==='equip'&&<>
