...
const NIVEAUX = [
  {v:-1,l:'Sous-sol'},{v:0,l:'RDC'},{v:1,l:'Étage 1'},{v:2,l:'Étage 2'},
  {v:10,l:'Jardin'},{v:11,l:'Cour'},{v:12,l:'Garage'},{v:13,l:'Terrasse'},
]
...
export default function PlanBien() {
  ...
  // Drag state pour déplacer les pièces et équipements
  const dragRef  = useRef(null) // { id, ox, oy }
  const dragERef = useRef(null) // { id, sx, sy, ox, oy }
  const svgRef   = useRef(null)

  // planRef pour accès synchrone (resize)
  ...
  // ── Drag pièce ────────────────────────────────────────
  function onPiecePointerDown(e, p) {
    e.stopPropagation()
    setSelP(p.id); setSelE(null)
    const r = svgRef.current?.getBoundingClientRect()
    if (!r) return
    const ox = Math.floor((e.clientX - r.left) / CELL) - p.x
    const oy = Math.floor((e.clientY - r.top) / CELL) - p.y
    dragRef.current = { id: p.id, ox, oy }
    e.currentTarget.setPointerCapture(e.pointerId)
  }

  function onPiecePointerMove(e, p) {
    if (!dragRef.current || dragRef.current.id !== p.id) return
    const { col, row } = getCell(e)
    setPlan(prev => prev.map(x => x.id === p.id
      ? { ...x, x: Math.max(0, Math.min(GW - x.w, col - dragRef.current.ox)), y: Math.max(0, Math.min(GH - x.h, row - dragRef.current.oy)) }
      : x
    ))
  }

  async function onPiecePointerUp(e, p) {
    if (!dragRef.current || dragRef.current.id !== p.id) return
    dragRef.current = null
    const cur = planRef.current.find(x => x.id === p.id)
    if (cur) await supabase.from('plan_pieces').update({ x: cur.x, y: cur.y }).eq('id', cur.id)
  }

  // ── Drag équipement ───────────────────────────────────
  function onEquipPointerDown(e, eq) {
    e.stopPropagation()
    setSelE(eq.id); setSelP(null)
    dragERef.current = { id: eq.id, sx: e.clientX, sy: e.clientY, ox: eq.grid_x||0, oy: eq.grid_y||0 }
    e.currentTarget.setPointerCapture(e.pointerId)
  }

  function onEquipPointerMove(e, eq) {
    if (!dragERef.current || dragERef.current.id !== eq.id) return
    const dx = e.clientX - dragERef.current.sx
    const dy = e.clientY - dragERef.current.sy
    setEquips(prev => prev.map(x => x.id === eq.id
      ? { ...x, grid_x: Math.max(0, Math.min(GW*CELL-24, dragERef.current.ox + dx)), grid_y: Math.max(0, Math.min(GH*CELL-24, dragERef.current.oy + dy)) }
      : x
    ))
  }

  async function onEquipPointerUp(e, eq) {
    if (!dragERef.current || dragERef.current.id !== eq.id) return
    dragERef.current = null
    const cur = equips.find(x => x.id === eq.id)
    if (cur) await supabase.from('plan_equipements').update({ grid_x: cur.grid_x, grid_y: cur.grid_y }).eq('id', cur.id)
  }

  // ── RESIZE avec Pointer Capture ───────────────────────
  // setPointerCapture garantit que tous les events pointer vont sur cet élément
  // même si la souris sort du SVG. Aucun listener document. Aucun conflit.
  const resizeRef = useRef(null) // { id, startX, startY, startW, startH }

  function onResizePointerDown(e, p) {
    e.stopPropagation()
    e.preventDefault()
    resizeRef.current = { id: p.id, startX: e.clientX, startY: e.clientY, startW: p.w, startH: p.h }
    e.currentTarget.setPointerCapture(e.pointerId)
  }
  ...
  function onResizePointerMove(e) {
    ...
    const dw = Math.round((e.clientX - startX) / CELL)
    const dh = Math.round((e.clientY - startY) / CELL)
    setPlan(prev => prev.map(p => p.id === id
      ? { ...p, w: Math.max(1, Math.min(GW - p.x, startW + dw)), h: Math.max(1, Math.min(GH - p.y, startH + dh)) }
      : p
    ))
  }
  ...
  async function onResizePointerUp(e) {
    ...
    const cur = planRef.current.find(p => p.id === id)
  ...
  async function addEquip(cat, eq, pid) {
    ...
    if (d) { setEquips(prev => [...prev, d]); setSelE(d.id) }
  }
  ...
  async function delPiece(pid) {
    ...
    setPlan(prev => prev.filter(p => p.id !== pid)); setSelP(null)
  }

  async function delEquip(eid) {
    await supabase.from('plan_equipements').delete().eq('id', eid)
    setEquips(prev => prev.filter(e => e.id !== eid)); setSelE(null)
  }
    ...
    <Layout>
      <div className="page-header" style={{ marginBottom:10 }}>
        ...
        <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
          <select className="btn btn-secondary btn-sm" value={niveau}
            onChange={e => setNiveau(Number(e.target.value))} style={{ cursor:'pointer' }}>
      ...
      {adding && (
        <div style={{ display:'flex', gap:8, marginBottom:10, alignItems:'center', padding:'9px 13px', background:'#E8F2EB', borderRadius:8 }}>
          <input autoFocus value={customNom} onChange={e => setCustomNom(e.target.value)}
            onKeyDown={e => e.key==='Enter' && addCustom()}
            placeholder="Nom de la pièce…"
            style={{ flex:1, padding:'7px 11px', border:'1px solid rgba(0,0,0,.15)', borderRadius:7, fontFamily:'inherit', fontSize:13, outline:'none' }}/>
      ...
      <div style={{ display:'flex', gap:10, height:'calc(100vh - 195px)', minHeight:450 }}>
        ...
        <div style={{ width:155, background:'#fff', border:'1px solid rgba(0,0,0,.08)', borderRadius:12, padding:8, overflowY:'auto', flexShrink:0 }}>
          <div style={{ display:'flex', gap:4, marginBottom:8 }}>
            {[['pieces','🏠 Pièces'],['equips','🔧 Équip.']].map(([t,l]) => (
              <button key={t} onClick={() => setTab(t)}
                style={{ flex:1, padding:'5px 4px', border:`1px solid ${tab===t?'#2D5A3D':'rgba(0,0,0,.1)'}`, borderRadius:7, background:tab===t?'#E8F2EB':'#F7F5F0', cursor:'pointer', fontSize:11, fontWeight:tab===t?600:400, color:tab===t?'#2D5A3D':'#6B6560' }}>
                {l}
              </button>
            ))}
          </div>
          ...
          {tab === 'equips' && <>
            <div style={{ fontSize:9, fontWeight:700, color:'#9E9890', textTransform:'uppercase', marginBottom:4 }}>
              {selPiece ? <span>→ <span style={{color:'#2D5A3D'}}>{selPiece.nom}</span></span> : 'Sélectionner une pièce'}
            </div>
        ...
        <div style={{ flex:1, overflow:'auto', background:'#F0EDE6', borderRadius:12, border:'1px solid rgba(0,0,0,.08)' }}>
          <svg ref={svgRef} width={GW*CELL} height={GH*CELL}
            style={{ display:'block', userSelect:'none', minWidth:GW*CELL, touchAction:'none' }}
            ...
            {plan.map(p => {
              ...
              return (
                <g key={p.id}
                  style={{ cursor: dragRef.current?.id === p.id ? 'grabbing' : 'grab', touchAction:'none' }}
                  onPointerDown={e => onPiecePointerDown(e, p)}
                  onPointerMove={e => onPiecePointerMove(e, p)}
                  ...
                  {isSel && <>
                    {/* Bouton supprimer */}
                    <g onClick={e => { e.stopPropagation(); delPiece(p.id) }} style={{ cursor:'pointer' }}>
                      <circle cx={px+pw-9} cy={py+9} r="9" fill="#B83232" opacity=".85"/>
                      <text x={px+pw-9} y={py+9} textAnchor="middle" dominantBaseline="middle" fontSize="11" fill="white" style={{pointerEvents:'none'}}>✕</text>
                    </g>

                    {/* ── POIGNÉE RESIZE — Pointer Capture ── */}
                    {/* Élément séparé avec ses propres handlers pointer */}
                    {/* stopPropagation empêche le parent (drag) de recevoir l'event */}
                    <g style={{ cursor:'se-resize', touchAction:'none' }}
                      onPointerDown={e => { e.stopPropagation(); onResizePointerDown(e, p) }}
                      onPointerMove={e => { e.stopPropagation(); onResizePointerMove(e) }}
                      onPointerUp={e => { e.stopPropagation(); onResizePointerUp(e) }}>
                      <rect x={px+pw-18} y={py+ph-18} width="16" height="16" rx="4"
                        fill="#2D5A3D" opacity=".85"/>
                      <text x={px+pw-10} y={py+ph-10} textAnchor="middle" dominantBaseline="middle"
            ...
            {equips.map(eq => {
              ...
              const isSel = selE === eq.id
              return (
                <g key={eq.id}
                  style={{ cursor: dragERef.current?.id === eq.id ? 'grabbing' : 'grab', touchAction:'none' }}
                  onPointerDown={e => onEquipPointerDown(e, eq)}
                  onPointerMove={e => onEquipPointerMove(e, eq)}
                  onPointerUp={e => onEquipPointerUp(e, eq)}
                  ...
                  {isSel && <>
                    ...
                    <g onClick={e => { e.stopPropagation(); delEquip(eq.id) }} style={{ cursor:'pointer' }}>
                      ...
                      <text x={ex+22} y={ey+2} textAnchor="middle" dominantBaseline="middle"
                        fontSize="9" fill="white" style={{pointerEvents:'none'}}>✕</text>
                    </g>
                  </>}
                </g>
              )
            ...
            {plan.length === 0 && equips.length === 0 && (
              <text x={GW*CELL/2} y={GH*CELL/2} textAnchor="middle" fontSize="14"
                fill="rgba(0,0,0,.2)" fontFamily="sans-serif">
                ← Cliquez sur une pièce dans la palette pour commencer
              </text>
            )}
          </svg>
        </div>
        ...
        <div style={{ width:155, background:'#fff', border:'1px solid rgba(0,0,0,.08)', borderRadius:12, padding:12, overflowY:'auto', flexShrink:0 }}>
          {selPiece && <>
            ...
            <div style={{ fontSize:10, color:'#9E9890', marginTop:8, lineHeight:1.8 }}>
            ...
            <div className="modal-body">
              ...
              <button className="btn btn-primary" onClick={async () => {
                if (modal.type==='piece') {
                  ...
                  setPlan(prev => prev.map(p => p.id===modal.id ? {...p,...u} : p))
                  ...
                  setEquips(prev => prev.map(e => e.id===modal.id ? {...e,...u} : e))
