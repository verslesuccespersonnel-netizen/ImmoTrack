...
const ECAT={
  ...
  'Électricité':[{n:'Prise',i:'🔌'},{n:'Interrupteur',i:'🔘'},{n:'Tableau élec.',i:'⚡'},{n:'Lumière',i:'💡'},{n:'VMC',i:'💨'},{n:'Sonnette',i:'🔔'},{n:'Interphone',i:'📞'},{n:'Climatisation',i:'❄️'},{n:'Détect. fumée',i:'🔴'}],
  'Plomberie':[{n:'Robinet',i:'🚰'},{n:'Évier',i:'🪣'},{n:'Lavabo',i:'🪣'},{n:'Baignoire',i:'🛁'},{n:'Douche',i:'🚿'},{n:'WC',i:'🚽'},{n:'Chasse d\'eau',i:'💧'},{n:'Lave-linge',i:'🫧'},{n:'Chauffe-eau',i:'💧'}],
  'Chauffage':[{n:'Radiateur',i:'🌡️'},{n:'Sèche-serviettes',i:'🌡️'},{n:'Chaudière',i:'🔥'},{n:'Thermostat',i:'🌡️'},{n:'Cheminée',i:'🔥'}],
  'Structure':[{n:'Mur',i:'🧱'},{n:'Plafond',i:'⬆️'},{n:'Sol / Parquet',i:'🟫'},{n:'Toiture',i:'🏠'},{n:'Façade',i:'🏠'},{n:'Garde-corps',i:'🚧'},{n:'Boîte lettres',i:'📬'}],
...
const PALCOLS=['#E8F2EB','#EBF2FC','#FDF3E7','#F3ECFC','#FDEAEA','#F0EDE6','#FDF6E3','#E8F8F5']

// Modes d'interaction
const MODE_NONE   = null
const MODE_DRAG   = 'drag'    // déplacer une pièce
const MODE_DRAG_E = 'drag_e'  // déplacer un équipement
const MODE_RESIZE = 'resize'  // redimensionner une pièce
...
export default function PlanBien() {
  ...
  const planRef    = useRef([])  // toujours à jour
  const equipsRef  = useRef([])  // toujours à jour
  const interRef   = useRef({ mode: MODE_NONE, id: null, ox:0, oy:0, sx:0, sy:0, sw:0, sh:0 })
  ...
  // ─── Listeners natifs sur le SVG ─────────────────────────
  // On utilise les events natifs (pas React) pour éviter tous les
  // problèmes de synthetic events, pooling, et currentTarget null.
  useEffect(() => {
    ...
    function getSVGPoint(e) {
      ...
      return {
        x: e.clientX - r.left,
        y: e.clientY - r.top,
        col: Math.max(0, Math.floor((e.clientX - r.left) / CELL)),
        row: Math.max(0, Math.floor((e.clientY - r.top) / CELL)),
      }
    }
    ...
    function onMove(e) {
      ...
      if (inter.mode === MODE_DRAG) {
        const newX = Math.max(0, Math.min(GW - inter.sw, pt.col - inter.ox))
        const newY = Math.max(0, Math.min(GH - inter.sh, pt.row - inter.oy))
        setPlan(prev => prev.map(p => p.id === inter.id ? {...p, x:newX, y:newY} : p))
      }

      if (inter.mode === MODE_DRAG_E) {
        const dx = e.clientX - inter.sx
        const dy = e.clientY - inter.sy
        setEquips(prev => prev.map(eq => eq.id === inter.id
          ? {...eq, grid_x: Math.max(0, Math.min(GW*CELL-24, inter.ox+dx)), grid_y: Math.max(0, Math.min(GH*CELL-24, inter.oy+dy))}
          : eq
        ))
      }

      if (inter.mode === MODE_RESIZE) {
        const dw = Math.round((e.clientX - inter.sx) / CELL)
        const dh = Math.round((e.clientY - inter.sy) / CELL)
        const cur = planRef.current.find(p => p.id === inter.id)
        if (!cur) return
        const nw = Math.max(1, Math.min(GW - cur.x, inter.sw + dw))
        const nh = Math.max(1, Math.min(GH - cur.y, inter.sh + dh))
        setPlan(prev => prev.map(p => p.id === inter.id ? {...p, w:nw, h:nh} : p))
      }
    }
    ...
    async function onUp(e) {
      ...
      if (inter.mode === MODE_DRAG) {
        const cur = planRef.current.find(p => p.id === inter.id)
        if (cur) await supabase.from('plan_pieces').update({x:cur.x, y:cur.y}).eq('id',cur.id)
      }
      if (inter.mode === MODE_DRAG_E) {
        const cur = equipsRef.current.find(eq => eq.id === inter.id)
        if (cur) await supabase.from('plan_equipements').update({grid_x:cur.grid_x, grid_y:cur.grid_y}).eq('id',cur.id)
      }
      if (inter.mode === MODE_RESIZE) {
        const cur = planRef.current.find(p => p.id === inter.id)
      ...
      interRef.current = { mode: MODE_NONE, id:null, ox:0, oy:0, sx:0, sy:0, sw:0, sh:0 }
    }
  ...
  }, []) // une seule fois — les refs sont toujours à jour

  function startDragPiece(e, p) {
    e.stopPropagation()
    setSelP(p.id); setSelE(null)
    const r = svgRef.current?.getBoundingClientRect()
    if (!r) return
    interRef.current = {
      mode: MODE_DRAG, id: p.id,
      ox: Math.floor((e.clientX - r.left) / CELL) - p.x,
      oy: Math.floor((e.clientY - r.top) / CELL) - p.y,
      sw: p.w, sh: p.h, sx:0, sy:0,
    }
    svgRef.current.setPointerCapture(e.nativeEvent.pointerId)
  }
  ...
  function startResize(e, p) {
    ...
    e.preventDefault()
    interRef.current = {
      mode: MODE_RESIZE, id: p.id,
      sx: e.clientX, sy: e.clientY,
      sw: p.w, sh: p.h,
      ox:0, oy:0,
    }
    // Capturer au niveau SVG — toujours disponible, jamais null
    svgRef.current.setPointerCapture(e.nativeEvent.pointerId)
  }
  ...
  async function addEquip(cat,eq,pid){
    ...
    if(d){setEquips(p=>[...p,d]);setSelE(d.id)}
  }
    ...
    <Layout>
      <div className="page-header" style={{marginBottom:10}}>
        ...
        <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
          <select className="btn btn-secondary btn-sm" value={niveau} onChange={e=>{setNiveau(Number(e.target.value))}} style={{cursor:'pointer'}}>
            {niveaux.map(n=><option key={n.v} value={n.v}>{n.l}</option>)}
          </select>
          <button className="btn btn-secondary btn-sm" onClick={()=>{const l=prompt('Nom du niveau');if(l){const v=100+niveaux.length;setNiveaux(ns=>[...ns,{v,l}]);setNiveau(v)}}}>+ Niveau</button>
        </div>
      </div>

      {adding&&<div style={{display:'flex',gap:8,marginBottom:10,alignItems:'center',padding:'9px 13px',background:'#E8F2EB',borderRadius:8}}>
        <input autoFocus value={customNom} onChange={e=>setCustomNom(e.target.value)} onKeyDown={e=>e.key==='Enter'&&addCustom()} placeholder="Nom de la pièce…" style={{flex:1,padding:'7px 11px',border:'1px solid rgba(0,0,0,.15)',borderRadius:7,fontFamily:'inherit',fontSize:13,outline:'none'}}/>
      ...
      <div style={{display:'flex',gap:10,height:'calc(100vh - 195px)',minHeight:450}}>
        ...
        {/* Palette gauche */}
        <div style={{width:155,background:'#fff',border:'1px solid rgba(0,0,0,.08)',borderRadius:12,padding:8,overflowY:'auto',flexShrink:0}}>
          <div style={{display:'flex',gap:4,marginBottom:8}}>
            {[['pieces','🏠'],['equips','🔧']].map(([t,ic])=>(
              <button key={t} onClick={()=>setTab(t)} style={{flex:1,padding:'5px 4px',border:`1px solid ${tab===t?'#2D5A3D':'rgba(0,0,0,.1)'}`,borderRadius:7,background:tab===t?'#E8F2EB':'#F7F5F0',cursor:'pointer',fontSize:11,fontWeight:tab===t?600:400,color:tab===t?'#2D5A3D':'#6B6560'}}>{ic} {t==='pieces'?'Pièces':'Équip.'}</button>
            ))}
          </div>
          ...
          {tab==='equips'&&<>
            <div style={{fontSize:9,fontWeight:700,color:'#9E9890',textTransform:'uppercase',marginBottom:4}}>{selPiece?<span>→ <span style={{color:'#2D5A3D'}}>{selPiece.nom}</span></span>:'Sélect. une pièce'}</div>
        ...
        <div style={{flex:1,overflow:'auto',background:'#F0EDE6',borderRadius:12,border:'1px solid rgba(0,0,0,.08)'}}>
          <svg
            ...
            style={{display:'block',userSelect:'none',minWidth:GW*CELL,touchAction:'none',cursor:'default'}}
            onClick={e=>{if(e.target===svgRef.current||e.target.tagName==='rect'&&e.target.getAttribute('fill')==='url(#grid)'){setSelP(null);setSelE(null)}}}>
            ...
            {plan.map(p=>{
              ...
              const isResizing=interRef.current.mode===MODE_RESIZE&&interRef.current.id===p.id
              return(
                <g key={p.id}>
                  {/* Zone principale : drag */}
                  <rect x={px+1} y={py+1} width={pw-2} height={ph-2} rx="6"
                    ...
                    opacity=".93"
                    style={{cursor:'grab'}}
                    onPointerDown={e=>startDragPiece(e,p)}
                  ...
                  {/* Bouton supprimer */}
                  {isSel&&<g onClick={e=>{e.stopPropagation();delPiece(p.id)}} style={{cursor:'pointer'}}>
                    <circle cx={px+pw-10} cy={py+10} r="10" fill="#B83232" opacity=".85"/>
                    <text x={px+pw-10} y={py+10} textAnchor="middle" dominantBaseline="middle" fontSize="12" fill="white" style={{pointerEvents:'none'}}>✕</text>
                  </g>}

                  {/* ── POIGNÉE RESIZE ──
                    Toujours visible quand sélectionné.
                    onPointerDown → startResize → setPointerCapture sur le SVG
                    Tous les move/up vont au SVG via les listeners natifs.
                  */}
                  {isSel&&(
                  ...
                  {/* Indicateur resize discret si non sélectionné */}
                  {!isSel&&(
                    <rect x={px+pw-8} y={py+ph-8} width="6" height="6" rx="1"
            ...
            {equips.map(eq=>{
              ...
              return(
                <g key={eq.id} style={{cursor:'grab'}}
                  onPointerDown={e=>startDragEquip(e,eq)}
                  ...
                  {isSel&&<>
                    ...
                    <g onClick={e=>{e.stopPropagation();delEquip(eq.id)}} style={{cursor:'pointer'}}>
                      <circle cx={ex+22} cy={ey+2} r="7" fill="#B83232" opacity=".85"/>
                      <text x={ex+22} y={ey+2} textAnchor="middle" dominantBaseline="middle" fontSize="9" fill="white" style={{pointerEvents:'none'}}>✕</text>
                    </g>
                  </>}
                </g>
              )
            ...
            {plan.length===0&&equips.length===0&&<text x={GW*CELL/2} y={GH*CELL/2} textAnchor="middle" fontSize="14" fill="rgba(0,0,0,.2)" fontFamily="sans-serif">← Cliquez sur une pièce dans la palette pour commencer</text>}
          </svg>
        </div>
        ...
        <div style={{width:155,background:'#fff',border:'1px solid rgba(0,0,0,.08)',borderRadius:12,padding:12,overflowY:'auto',flexShrink:0}}>
          {selPiece&&<>
            <div style={{fontSize:28,margin:'4px 0'}}>{selPiece.icone}</div>
            ...
            <button className="btn btn-sm" style={{background:'#E8F2EB',color:'#2D5A3D',border:'none',width:'100%',marginBottom:4}}
              onClick={()=>{setForm({nom:selPiece.nom,icone:selPiece.icone,couleur:selPiece.couleur,w:selPiece.w,h:selPiece.h});setModal({type:'piece',id:selPiece.id})}}>✏️ Modifier</button>
            <div style={{fontSize:10,color:'#9E9890',marginTop:8,lineHeight:1.9,background:'#F7F5F0',borderRadius:8,padding:'8px 10px'}}>
              🖱️ Glisser = déplacer<br/>
              ↘ Coin vert = redimensionner<br/>
              Double-clic = éditer<br/>
              ✕ = supprimer
            </div>
          </>}
          {selEquip&&<>
            <div style={{fontSize:28,margin:'4px 0'}}>{selEquip.icone}</div>
            ...
            <button className="btn btn-sm" style={{background:'#E8F2EB',color:'#2D5A3D',border:'none',width:'100%'}}
              onClick={()=>{setForm({nom:selEquip.nom,icone:selEquip.icone,notes:selEquip.notes||'',piece_id:selEquip.plan_piece_id||''});setModal({type:'equip',id:selEquip.id})}}>✏️ Modifier</button>
          </>}
          {!selPiece&&!selEquip&&<>
            <div style={{fontSize:10,color:'#9E9890',lineHeight:2,marginBottom:10}}>
            ...
            {plan.map(p=><div key={p.id} onClick={()=>setSelP(p.id)} style={{display:'flex',alignItems:'center',gap:5,padding:'3px 4px',borderRadius:5,cursor:'pointer',background:selP===p.id?'#E8F2EB':'transparent',marginBottom:2}}>
      ...
      {modal&&<div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setModal(null)}>
        <div className="modal" style={{maxWidth:360}}>
          ...
          <div className="modal-body">
            ...
            {modal.type==='piece'&&<>
              <div className="grid2"><div className="fld"><label>Largeur</label><input type="number" min="1" max={GW} value={form.w||''} onChange={e=>set('w',e.target.value)}/></div><div className="fld"><label>Hauteur</label><input type="number" min="1" max={GH} value={form.h||''} onChange={e=>set('h',e.target.value)}/></div></div>
              <div className="fld"><label>Couleur</label><div style={{display:'flex',gap:5,flexWrap:'wrap',marginTop:4}}>{PALCOLS.map(c=><div key={c} onClick={()=>set('couleur',c)} style={{width:24,height:24,borderRadius:5,background:c,cursor:'pointer',border:`2.5px solid ${form.couleur===c?'#2D5A3D':'rgba(0,0,0,.12)'}`}}/>)}</div></div>
            </>}
            {modal.type==='equip'&&<>
