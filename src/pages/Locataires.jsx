...
export default function Locataires() {
  ...
  async function load() {
    ...
    try {
      ...
      if (profile?.role === 'admin') {
        biensQuery = supabase.from('biens').select('id,adresse,ville,surface_m2,type_bien')
      } else {
        biensQuery = supabase.from('biens').select('id,adresse,ville,surface_m2,type_bien').eq('proprietaire_id', session.user.id)
      }
      ...
      const filtered = profile?.role === 'admin'
        ? (lr.data || [])
        : (lr.data || []).filter(l => myIds.has(l.bien_id))
  ...
  async function addGarant() {
    if (!form.g_nom || !form.g_prenom) {
      setFormErr('Nom et prenom requis')
      return
    }
    setSaving(true)
    try {
  ...
  if (error) return (
    <Layout>
      <div className="it-center">
        <div className="alert alert-error" style={{maxWidth:320, textAlign:'center'}}>
          {error}
          <br/>
            ...
            <div className="modal-body">
              ...
              {modal.type === 'create' && (
                <>
                  ...
                  <div className="grid2">
                    <div className="fld">
                      <label>Loyer HC (euros) *</label>
                      <input type="number" value={form.loyer||''} onChange={e=>set('loyer',e.target.value)}/>
                    </div>
                    <div className="fld">
                      <label>Charges (euros/mois)</label>
                      <input type="number" value={form.charges||''} onChange={e=>set('charges',e.target.value)}/>
                    </div>
                  </div>
                  <div className="fld">
                    <label>Depot de garantie (euros)</label>
                    <input type="number" value={form.depot_garantie||''} onChange={e=>set('depot_garantie',e.target.value)}/>
                  </div>
                  <div className="grid2">
                    <div className="fld">
                    ...
                    <div className="fld">
                      <label>Date sortie prevue</label>
                  ...
                  <div className="grid2">
                    <div className="fld">
                      ...
                      <input type="number" value={form.surface_m2||''} onChange={e=>set('surface_m2',e.target.value)}/>
                    </div>
                    <div className="fld">
                      <label>Nb pieces</label>
              ...
              {modal.type === 'garant' && (
                <>
                  <div className="grid2">
                    <div className="fld">
                      <label>Prenom *</label>
                      <input value={form.g_prenom||''} onChange={e=>set('g_prenom',e.target.value)}/>
                    </div>
                    <div className="fld">
                      <label>Nom *</label>
                      <input value={form.g_nom||''} onChange={e=>set('g_nom',e.target.value)}/>
                    </div>
                  </div>
                  <div className="grid2">
                    <div className="fld">
                      <label>Telephone</label>
                      <input value={form.g_tel||''} onChange={e=>set('g_tel',e.target.value)}/>
                    </div>
                    <div className="fld">
                      <label>Email</label>
                      <input type="email" value={form.g_email||''} onChange={e=>set('g_email',e.target.value)}/>
                    </div>
                  </div>
                  <div className="fld">
                    <label>Lien avec le locataire</label>
                    <select value={form.g_lien||'autre'} onChange={e=>set('g_lien',e.target.value)}>
                      {['parent','conjoint','ami','employeur','organisme','autre'].map(l => (
                  ...
                  <div className="fld">
                    <label>Montant garanti (euros)</label>
                    <input type="number" value={form.g_montant||''} onChange={e=>set('g_montant',e.target.value)}/>
                  </div>
              ...
              {modal.type === 'occupant' && (
                <>
                  <div className="grid2">
                    <div className="fld">
                      <label>Prenom *</label>
                      <input value={form.o_prenom||''} onChange={e=>set('o_prenom',e.target.value)}/>
                    </div>
                    <div className="fld">
                      <label>Nom *</label>
                      <input value={form.o_nom||''} onChange={e=>set('o_nom',e.target.value)}/>
                    </div>
                  </div>
                  <div className="grid2">
                    <div className="fld">
                      <label>Date de naissance</label>
                      <input type="date" value={form.o_dob||''} onChange={e=>set('o_dob',e.target.value)}/>
                    </div>
                    <div className="fld">
                      <label>Lien</label>
                      <select value={form.o_lien||'autre'} onChange={e=>set('o_lien',e.target.value)}>
                        {['titulaire','conjoint','enfant','colocataire','autre'].map(l => (
                          <option key={l}>{l}</option>
                        ))}
                      </select>
                    </div>
                  </div>
                  <button className="btn btn-primary" onClick={addOccupant}>
                    Ajouter
                  </button>
                </>
              )}
...
function LocCard({ loc, archived, expanded, setExpanded, setModal, setForm, setFormErr, archiver, supprimer, delGarant, delOccupant }) {
  ...
  const nom   = p
    ? (p.prenom + ' ' + p.nom)
    : t
    ? (t.prenom + ' ' + t.nom + ' (compte non cree)')
    : 'Sans compte'
  ...
  const typeLabel = {
    ...
    courte_duree: 'Courte duree',
    colocation: 'Colocation',
    autre: 'Autre',
  }[loc.type_contrat] || loc.type_contrat
  ...
  return (
    <div className="card" style={{marginBottom:10, opacity: archived ? 0.75 : 1}}>
      <div
        ...
        onClick={() => setExpanded(isExp ? null : loc.id)}
      >
        <div style={{display:'flex', alignItems:'center', gap:10}}>
          <div style={{
            width:36, height:36, borderRadius:'50%',
            background: archived ? '#F7F5F0' : '#EBF2FC',
            color: archived ? '#9E9890' : '#2B5EA7',
          ...
          <div>
            ...
            <div style={{fontSize:12, color:'#6B6560'}}>
              ...
              {loc.loyer_mensuel
                ...
                : ''}
              {loc.date_debut
                ? ' - entree ' + new Date(loc.date_debut).toLocaleDateString('fr-FR')
                : ''}
            </div>
          </div>
        </div>
        <div style={{display:'flex', alignItems:'center', gap:6, flexWrap:'wrap'}}>
          ...
          {typeLabel && (
            <span className="status status-blue" style={{fontSize:10}}>{typeLabel}</span>
          )}
        ...
        <div className="card-body">
          {p && (
            <div style={{
              ...
              background:'#F7F5F0', padding:'8px 12px', borderRadius:8,
            }}>
          ...
          {loc.depot_garantie > 0 && (
            <div style={{
              marginBottom:10, background:'#FDF3E7',
          ...
          {(loc.occupants || []).length > 0 && (
            <div style={{marginBottom:10}}>
              <div style={{
                fontSize:10, fontWeight:700, color:'#9E9890',
                textTransform:'uppercase', marginBottom:5,
              }}>Occupants</div>
              {loc.occupants.map(o => (
                <div key={o.id} style={{
                  display:'flex', justifyContent:'space-between',
                  padding:'3px 0', borderBottom:'1px solid rgba(0,0,0,.04)', fontSize:13,
                }}>
                  <span>
                    {o.prenom} {o.nom}
                    <span style={{color:'#9E9890', fontSize:11, marginLeft:6}}>
                      ({o.lien})
                      {o.date_naissance
                        ? ' - ' + (new Date().getFullYear() - new Date(o.date_naissance).getFullYear()) + ' ans'
                        : ''}
                    </span>
                  </span>
                  <button
                    onClick={() => delOccupant(o.id)}
                    style={{background:'none', border:'none', cursor:'pointer', color:'#B83232', fontSize:13}}
                  >X</button>
                </div>
              ))}
            </div>
          ...
          {(loc.garants || []).length > 0 && (
            <div style={{marginBottom:10}}>
              <div style={{
                ...
                Garants ({loc.garants.length})
              </div>
              {loc.garants.map(g => (
                <div key={g.id} style={{
                  display:'flex', justifyContent:'space-between',
                  padding:'3px 0', borderBottom:'1px solid rgba(0,0,0,.04)',
                }}>
          ...
          <div style={{
            ...
            paddingTop:8, borderTop:'1px solid rgba(0,0,0,.07)',
          }}>
            {!archived && (
              <>
                <button className="btn btn-secondary btn-sm" onClick={() => {
                  setFormErr('')
                  setForm({})
                  setModal({type:'occupant', locationId: loc.id})
                }}>+ Occupant</button>
                <button className="btn btn-secondary btn-sm" onClick={() => {
                  setFormErr('')
                  setForm({})
                  setModal({type:'garant', locationId: loc.id})
                }}>+ Garant</button>
                <button className="btn btn-danger btn-sm" onClick={() => archiver(loc.id)}>
                  Archiver
                </button>
              </>
            )}
            <button className="btn btn-danger btn-sm" onClick={() => supprimer(loc.id)}>
              Supprimer
            </button>
          </div>
        </div>
