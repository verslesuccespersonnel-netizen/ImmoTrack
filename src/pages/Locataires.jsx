...
const CONTRATS = [
  {v:'bail_vide',    l:'Bail vide'},
  {v:'bail_meuble',  l:'Bail meuble'},
  {v:'bail_commercial', l:'Bail commercial'},
  {v:'courte_duree', l:'Courte duree / Airbnb'},
...
export default function Locataires() {
  ...
  const [orphelins,   setOrphelins]  = useState([]) // profils locataires sans location
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // 1. Charger les biens du proprio (ou tous si admin)
      const biensRes = isAdmin
        ? await supabase.from('biens').select('id,adresse,ville,surface_m2,type_bien')
        : await supabase.from('biens').select('id,adresse,ville,surface_m2,type_bien').eq('proprietaire_id', session.user.id)

      const myBiens = biensRes.data || []
      setBiens(myBiens)
      const myBienIds = myBiens.map(b => b.id)

      // 2. Charger les locations liées à ces biens
      let locsData = []
      if (myBienIds.length > 0) {
        const locsRes = await supabase
          .from('locations')
          .select(`
            *,
      ...
      // 3. Charger les profils locataires SANS location active
      // (locataires qui existent mais ne sont pas encore attribués à un bien)
      ...
      const locWithProfile = new Set(locsData.map(l => l.locataire_id).filter(Boolean))
      const orphans = (allProfs.data || []).filter(p => !locWithProfile.has(p.id))
      setOrphelins(orphans)
  ...
  // Auto-remplir depuis le bien sélectionné
  useEffect(() => {
    if (!form.bien_id) return
    const bien = biens.find(b => b.id === form.bien_id)
    if (bien?.surface_m2 && !form.surface_m2) {
  ...
  async function createLocataire() {
    ...
    if (!form.bien_id)             errors.push('Bien')
    if (!form.loyer)               errors.push('Loyer')
    if (!form.date_debut)          errors.push('Date entree')
    if (errors.length > 0) {
      setFormErr('Champs obligatoires : ' + errors.join(', '))
      return
    }
    ...
    try {
      ...
      const { error: occErr } = await supabase.from('occupants').insert({
        location_id:    loc.id,
        nom:            form.nom,
        prenom:         form.prenom,
        lien:           'titulaire',
      ...
      if (occErr) console.warn('occupant:', occErr.message)

      // Invitation si email fourni et pas de compte
  ...
  async function attribuerOrphelin(profilId) {
    // Attribuer un profil existant à un bien
    setForm({ user_id: profilId })
  ...
  async function supprimer(id) {
    if (!window.confirm('Supprimer definitivement ?')) return
    await supabase.from('locations').delete().eq('id', id)
    await load()
  }

  async function delGarant(id)   { await supabase.from('garants').delete().eq('id', id); await load() }
  async function delOccupant(id) { await supabase.from('occupants').delete().eq('id', id); await load() }

  const actifs   = locations.filter(l => l.statut === 'actif')
  const archives = locations.filter(l => l.statut !== 'actif')
  ...
  return (
    <Layout>
      <div className="page-header">
        <div>
          ...
          <p className="page-sub">
            {actifs.length} location(s) active(s)
            {orphelins.length > 0 ? ` - ${orphelins.length} compte(s) sans logement` : ''}
      ...
      {/* Locations actives */}
      {actifs.length === 0 && orphelins.length === 0 && (
        <div className="card">
          <div className="card-body" style={{textAlign:'center', padding:40, color:'#9E9890'}}>
            Aucun locataire. Utilisez le bouton Ajouter ou attribuez un locataire depuis la page Biens.
          </div>
        </div>
      )}
      ...
      {/* Comptes locataires sans logement attribué */}
      {orphelins.length > 0 && (
        <>
          <div style={{
            fontSize:12, fontWeight:600, color:'#C8813A',
            textTransform:'uppercase', margin:'20px 0 10px',
            display:'flex', alignItems:'center', gap:8,
          }}>
            <span>Comptes sans logement attribue ({orphelins.length})</span>
            <div style={{flex:1, height:1, background:'rgba(0,0,0,.08)'}}/>
          </div>
          <div className="alert alert-warn" style={{marginBottom:12, fontSize:13}}>
            Ces locataires ont un compte mais ne sont pas encore rattaches a un bien.
            Cliquez sur un compte ou allez dans Biens pour les attribuer.
          </div>
          {orphelins.map(p => (
            <div key={p.id} className="card" style={{marginBottom:8}}>
              <div className="row-item">
                <div style={{
                  ...
                  background:'#FDF3E7', color:'#C8813A',
                ...
                <div style={{flex:1}}>
                  <div style={{fontWeight:500, fontSize:13}}>{p.prenom} {p.nom}</div>
                  <div style={{fontSize:11, color:'#9E9890'}}>
                    {p.telephone || ''}{p.email ? ' - ' + p.email : ''}
                    {' - Cree le ' + new Date(p.created_at).toLocaleDateString('fr-FR')}
                  </div>
                </div>
                <span className="status status-yellow">Sans logement</span>
                <button
                  className="btn btn-primary btn-sm"
                  onClick={() => {
                    setFormErr('')
                    setForm({ user_id: p.id, nom: p.nom, prenom: p.prenom, telephone: p.telephone || '', email: p.email || '' })
                    setModal({type:'create'})
                  }}
                >
                  Attribuer un bien
      ...
      {showArchives && archives.length > 0 && (
        <>
          <div style={{
            fontSize:12, fontWeight:600, color:'#9E9890',
            textTransform:'uppercase', margin:'20px 0 10px',
            display:'flex', alignItems:'center', gap:8,
          }}>
            ...
            <div className="modal-body">
              ...
              {modal.type === 'create' && (
                <>
                  ...
                  <div className="grid2">
                    <div className="fld"><label>Date naissance</label><input type="date" value={form.dob||''} onChange={e=>set('dob',e.target.value)}/></div>
                    <div className="fld"><label>UUID compte (si existant)</label><input value={form.user_id||''} onChange={e=>set('user_id',e.target.value)} placeholder="Laisser vide sinon"/></div>
                  </div>
                  {form.user_id && (
                    <div className="alert alert-success" style={{fontSize:12}}>
                      Ce locataire sera lie au compte existant {form.user_id.slice(0,8)}...
                    </div>
                  )}
                  <div className="fld">
                    <label>Bien *</label>
                    ...
                    {biens.length === 0 && (
                      <div style={{fontSize:12, color:'#B83232', marginTop:4}}>
                        Aucun bien disponible. Creez d'abord un bien dans le menu Biens.
                      </div>
                    )}
                  </div>
                  <div className="fld">
                  ...
                  <div style={{display:'flex', gap:16, flexWrap:'wrap', padding:'6px 0'}}>
                    {[['meuble','Meuble'],['ascenseur','Ascenseur'],['parking','Parking'],['cave','Cave']].map(([k,l]) => (
                      <label key={k} style={{display:'flex', alignItems:'center', gap:6, cursor:'pointer', fontSize:13}}>
                        <input type="checkbox" checked={form[k]||false} onChange={e=>set(k,e.target.checked)}/>{l}
                      </label>
                    ))}
                  </div>
                  ...
                  <button className="btn btn-primary" onClick={createLocataire} disabled={saving}>
                    {saving ? 'Creation en cours...' : 'Creer le locataire'}
                  </button>
                </>
              )}
                ...
                <>
                  ...
                  <div className="fld">
                    ...
                    <select value={form.g_type||'physique'} onChange={e=>set('g_type',e.target.value)}>
                      ...
                      <option value="bancaire">Caution bancaire</option>
                    </select>
                  </div>
                  ...
                  <button className="btn btn-primary" onClick={addGarant} disabled={saving}>{saving ? '...' : 'Ajouter'}</button>
                </>
              )}
...
function LocCard({ loc, archived, expanded, setExpanded, setModal, setForm, setFormErr, archiver, supprimer, delGarant, delOccupant }) {
  ...
  const nom   = p
    ...
    : t
    ? (t.prenom + ' ' + t.nom)
    : 'Locataire sans profil'
  const sansCompte = !p

  const typeLabel = CONTRATS.find(c => c.v === loc.type_contrat)?.l || loc.type_contrat || ''
    ...
    <div className="card" style={{marginBottom:10, opacity: archived ? 0.75 : 1}}>
      <div className="card-header" style={{cursor:'pointer'}} onClick={() => setExpanded(isExp ? null : loc.id)}>
        <div style={{display:'flex', alignItems:'center', gap:10}}>
          <div style={{
            width:36, height:36, borderRadius:'50%',
            background: archived ? '#F7F5F0' : (sansCompte ? '#FDF6E3' : '#EBF2FC'),
            color: archived ? '#9E9890' : (sansCompte ? '#C8813A' : '#2B5EA7'),
            display:'flex', alignItems:'center', justifyContent:'center',
            fontSize:13, fontWeight:700, flexShrink:0,
          }}>
            {nom[0]}{nom.split(' ')?.[1]?.[0] || ''}
          ...
          <div>
            <div style={{fontWeight:600, fontSize:14}}>
              {nom}
              {sansCompte && <span style={{fontSize:11, color:'#C8813A', marginLeft:6}}>compte non cree</span>}
            </div>
            <div style={{fontSize:12, color:'#6B6560'}}>
              {loc.biens?.adresse || 'Bien inconnu'}, {loc.biens?.ville || ''}
        ...
        <div style={{display:'flex', alignItems:'center', gap:6, flexWrap:'wrap'}}>
          <span className={'status ' + (loc.statut === 'actif' ? 'status-green' : 'status-grey')}>
            {loc.statut}
          </span>
        ...
        <div className="card-body">
          ...
          {loc.depot_garantie > 0 && (
            <div style={{marginBottom:10, background:'#FDF3E7', borderRadius:8, padding:'8px 12px', fontSize:13}}>
              Depot de garantie : {Number(loc.depot_garantie).toLocaleString('fr-FR')} euros
            </div>
          )}
          ...
          {(loc.garants||[]).length > 0 && (
            <div style={{marginBottom:10}}>
              ...
              {loc.garants.map(g => (
                <div key={g.id} style={{display:'flex', justifyContent:'space-between', padding:'3px 0', borderBottom:'1px solid rgba(0,0,0,.04)'}}>
