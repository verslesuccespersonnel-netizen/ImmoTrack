...
const SPE = ['Plomberie','Électricité','Chauffage / Climatisation','Serrurerie',
...
const ICO = {'Plomberie':'🚰','Électricité':'⚡','Chauffage / Climatisation':'🌡️',
...
export default function Prestataires() {
  ...
  async function load() {
    ...
    try {
      const { data, error: e } = await supabase
        .from('prestataires').select('*').order('nom')
      if (e) throw e
      setItems(data || [])
    } catch(e) {
      setError(e.message)
  ...
  async function save() {
    ...
    try {
      const pl = {
        nom: form.nom, prenom: form.prenom||null, societe: form.societe||null,
        telephone: form.telephone||null, email: form.email||null,
        specialite: form.specialite||null, adresse: form.adresse||null, notes: form.notes||null
      }
  ...
  if (error)   return <Layout><div className="it-center">
    <div className="alert alert-error" style={{maxWidth:320,textAlign:'center'}}>
      {error}<br/><button className="btn btn-secondary btn-sm" style={{marginTop:8}} onClick={load}>↺ Réessayer</button>
    </div>
  </div></Layout>
    ...
    <Layout>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:12, flexWrap:'wrap' }}>
        ...
        <select style={{ padding:'7px 12px', border:'1px solid rgba(0,0,0,.15)', borderRadius:8, fontFamily:'inherit', fontSize:13, outline:'none', background:'#fff' }}
          ...
          {[...new Set(items.map(p => p.specialite).filter(Boolean))].map(s =>
      ...
      {filtered.length === 0 && (
        <div className="card"><div className="card-body" style={{ textAlign:'center', padding:40, color:'#9E9890' }}>
          {items.length === 0 ? 'Ajoutez vos artisans de confiance.' : 'Aucun résultat.'}
        </div></div>
      )}
            ...
            <div className="card-body" style={{ padding:14 }}>
              ...
              {p.specialite && (
                <div style={{ marginBottom:8 }}>
                  <span style={{ padding:'3px 9px', borderRadius:20, fontSize:11, fontWeight:600, background:'#FDF3E7', color:'#C8813A' }}>
              ...
              <div style={{ display:'flex', gap:5, marginTop:10 }}>
                <button className="btn btn-secondary btn-sm" style={{ flex:1 }}
                  onClick={() => { setForm(p); setFormErr(''); setModal(p) }}>✏️</button>
            ...
            <div className="modal-body">
              ...
              <div style={{ display:'flex', gap:6 }}>
                <input style={{ flex:1, padding:'7px 10px', border:'1px solid rgba(0,0,0,.15)', borderRadius:7, fontFamily:'inherit', fontSize:12, outline:'none' }}
                  ...
                  onKeyDown={e => { if(e.key==='Enter' && newSpe.trim()) { set('specialite',newSpe.trim()); setNewSpe('') }}}/>
                <button className="btn btn-secondary btn-sm"
                  onClick={() => { if(newSpe.trim()) { set('specialite',newSpe.trim()); setNewSpe('') } }}>+ Ajouter</button>
              </div>
