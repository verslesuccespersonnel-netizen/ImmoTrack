...
export default function Prestataires() {
  ...
  // Guard session dans le fetcher
  const { data: items=[], loading, error, reload } = useLoad(async () => {
    if (!session?.user) return []
    const { data, error: e } = await supabase.from('prestataires').select('*').order('nom')
    if (e) throw e
    return data || []
  }, [session?.user?.id])
  ...
  return (
    <Layout>
      <div className="page-header">
        <div><h1 className="page-title">Prestataires</h1><p className="page-sub">{items.length} prestataire(s)</p></div>
        <button className="btn btn-primary" onClick={()=>{setForm({});setFormErr('');setModal({})}}>+ Ajouter</button>
      </div>
      ...
      <div style={{display:'flex',gap:8,marginBottom:12,flexWrap:'wrap'}}>
        ...
        <select style={{padding:'7px 12px',border:'1px solid rgba(0,0,0,.15)',borderRadius:8,fontFamily:'inherit',fontSize:13,outline:'none',background:'#fff'}} value={filterSpe} onChange={e=>setFilter(e.target.value)}>
          <option value="">Toutes spécialités</option>
          {[...new Set(items.map(p=>p.specialite).filter(Boolean))].map(s=><option key={s} value={s}>{SPE_ICONS[s]||'🔧'} {s}</option>)}
        </select>
      </div>

      {filtered.length===0 && <div className="card"><div className="card-body" style={{textAlign:'center',padding:40,color:'#9E9890'}}>{items.length===0?'Ajoutez vos artisans de confiance.':'Aucun résultat.'}</div></div>}
            ...
            <div className="card-body" style={{padding:14}}>
              <div style={{display:'flex',alignItems:'flex-start',gap:10,marginBottom:8}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontWeight:600,fontSize:14,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{p.prenom?`${p.prenom} ${p.nom}`:p.nom}</div>
                  {p.societe&&<div style={{fontSize:12,color:'#6B6560'}}>{p.societe}</div>}
                </div>
              </div>
              {p.specialite&&<div style={{marginBottom:8}}><span style={{padding:'3px 9px',borderRadius:20,fontSize:11,fontWeight:600,background:'#FDF3E7',color:'#C8813A'}}>{SPE_ICONS[p.specialite]||'🔧'} {p.specialite}</span></div>}
          ...
          <div className="modal">
            ...
            <div className="modal-body">
              ...
              <div style={{display:'flex',gap:6}}>
                <input style={{flex:1,padding:'7px 10px',border:'1px solid rgba(0,0,0,.15)',borderRadius:7,fontFamily:'inherit',fontSize:12,outline:'none'}} placeholder="Nouvelle spécialité…" value={newSpe} onChange={e=>setNewSpe(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&newSpe.trim()){set('specialite',newSpe.trim());setNewSpe('')}}}/>
