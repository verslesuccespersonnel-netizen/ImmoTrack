...
const STATUTS = ['nouveau','en_cours','en_attente','resolu','annule']
const GRAVITES = ['faible','moyen','urgent']
...
export default function Incidents() {
  ...
  useEffect(() => { if (session) load() }, [session, profile])
  ...
  async function updateStatut(id, statut) {
    ...
    setItems(items.map(i => i.id===id ? {...i, statut} : i))
  ...
  return (
    <Layout>
      ...
      {filtered.length===0 && <div className="card"><div className="card-body" style={{textAlign:'center',color:'#9E9890',padding:32}}>✅ Aucun incident{filter!=='tous'?' pour ce filtre':''}</div></div>}

      {filtered.map(inc => (
        <div key={inc.id} className="card" style={{ marginBottom:10, cursor:'pointer' }} onClick={()=>setSel(inc)}>
          <div className="row-item">
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ fontSize:11, color:'#9E9890' }}>
                {inc.biens?.adresse} · {inc.profiles?.prenom} {inc.profiles?.nom} · {new Date(inc.created_at).toLocaleDateString('fr-FR')}
                {inc.medias?.length>0 ? ` · 📎 ${inc.medias.length}` : ''}
              </div>
            </div>
        ...
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setSel(null)}>
          <div className="modal modal-lg">
            <div className="modal-header">
              <span className="modal-title">{sel.titre}</span>
