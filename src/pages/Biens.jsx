...
export default function Biens() {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([
      supabase.from('biens')
        .select('*, locations(id, statut, loyer_mensuel, date_debut, date_fin, type_contrat, profiles!locataire_id(id, nom, prenom, telephone))')
    ...
    .then(([biensRes, locsRes]) => {
      clearTimeout(timeout)
      if (ctrl.signal.aborted) return
      setBiens(biensRes.data || [])

      // Trier : locataires sans location active en premier
  ...
  // Reload quand l'onglet redevient visible (bfcache fix)
  useEffect(() => {
    function onVisible() {
  ...
  async function saveBien() {
    if (!form.adresse || !form.ville) { setFormError('Adresse et ville requises'); return }
    setSaving(true); setFormError('')
    try {
      if (modal.id) {
        await supabase.from('biens').update({
          adresse: form.adresse, ville: form.ville,
          code_postal: form.code_postal || null, type_bien: form.type_bien || null,
  ...
  async function attribuerLocataire() {
    if (!form.loc_id || !form.loyer || !form.date_debut) {
      setFormError('Locataire, loyer et date d\'entrée requis'); return
    }
    setSaving(true); setFormError('')
    try {
      ...
      if (locActive) {
        await supabase.from('locations').update({
          statut: 'termine', date_fin: new Date().toISOString().split('T')[0]
        }).eq('id', locActive.id)
      }
      await supabase.from('locations').insert({
        bien_id: attrib.id, locataire_id: form.loc_id,
        loyer_mensuel: Number(form.loyer), date_debut: form.date_debut,
        date_fin: form.date_fin || null, type_contrat: form.type_contrat || null,
        statut: 'actif',
      })
      setAttrib(null)
      load()
  ...
  if (loading) return (
    <Layout>
      <div className="it-center">
        ...
        <div style={{ color:'#9E9890', fontSize:13, marginTop:10 }}>Chargement des biens…</div>
      </div>
    </Layout>
  )
  ...
  if (error) return (
    <Layout>
      <div className="it-center" style={{ flexDirection:'column', gap:12 }}>
        <div className="alert alert-error" style={{ maxWidth:360 }}>⚠️ {error}</div>
    ...
    <Layout>
      <div className="page-header">
        <div>
          ...
          <p className="page-sub">{biens.length} bien(s) · {biens.filter(b => b.locations?.some(l => l.statut==='actif')).length} occupé(s)</p>
        </div>
        <button className="btn btn-primary" onClick={() => { setForm({}); setFormError(''); setModal({}) }}>
          + Ajouter un bien
        </button>
      </div>
          ...
          <div key={b.id} className="card" style={{ marginBottom:12 }}>
            <div className="card-header">
              ...
              <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                ...
                <button className="btn btn-secondary btn-sm" onClick={() => { setFormError(''); setAttrib(b); setForm({}) }}>+ Locataire</button>
                <button className="btn btn-secondary btn-sm" onClick={() => {
                  setForm({ adresse:b.adresse, ville:b.ville, code_postal:b.code_postal, type_bien:b.type_bien, surface_m2:b.surface_m2 })
            ...
            <div className="card-body" style={{ paddingTop:10, paddingBottom:12 }}>
              {locsActives.length === 0 ? (
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  ...
                  <button className="btn btn-secondary btn-sm" onClick={() => { setFormError(''); setAttrib(b); setForm({}) }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  {locsActives.map(loc => (
                    <div key={loc.id} style={{ display:'flex', alignItems:'center', gap:10, flexWrap:'wrap' }}>
                      ...
                      {loc.profiles ? (
                        <span onClick={() => navigate('/locataires')}
                          style={{ fontSize:13, color:'#2D5A3D', fontWeight:500, cursor:'pointer', textDecoration:'underline dotted' }}>
                          👤 {loc.profiles.prenom} {loc.profiles.nom}
                        </span>
                      ) : (
                        <span style={{ fontSize:12, color:'#9E9890' }}>⚠️ En attente du compte locataire</span>
                      )}
                      ...
                      {loc.date_debut && <span style={{ fontSize:11, color:'#9E9890' }}>depuis {new Date(loc.date_debut).toLocaleDateString('fr-FR')}</span>}
            ...
            <div className="modal-body">
              ...
              <div className="fld">
                ...
                <select value={form.loc_id||''} onChange={e=>set('loc_id',e.target.value)}>
                  ...
                  {locataires.map(l => (
                    <option key={l.id} value={l.id}>{l.prenom} {l.nom}{l.telephone ? ` · ${l.telephone}` : ''}</option>
                  ))}
                </select>
                <div style={{ fontSize:11, color:'#9E9890', marginTop:3 }}>
                  Si le locataire n'apparaît pas, créez-le d'abord dans le menu Locataires.
                </div>
              </div>
              <div className="grid2">
              ...
              <div className="grid2">
                ...
                <div className="fld"><label>Type de contrat</label>
                  <select value={form.type_contrat||''} onChange={e=>set('type_contrat',e.target.value)}>
                    <option value="">— Choisir —</option>
                    {CONTRATS.map(v => <option key={v} value={v}>{v.replace(/_/g,' ')}</option>)}
                  </select>
                </div>
              </div>
              <button className="btn btn-primary" onClick={attribuerLocataire} disabled={saving}>
                {saving ? '…' : '👤 Attribuer'}
              </button>
            </div>
          </div>
