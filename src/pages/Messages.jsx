...
export default function Messages() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [{ data: allProfiles }, { data: msgs }] = await Promise.all([
      ...
      console.error('Messages load:', e.message)
    } finally {
      setLoading(false)
    }
  }, [session?.user?.id])
  ...
  useEffect(() => {
    loadAll()
    const sub = supabase.channel('msgs_rt')
  ...
  const convList = Object.entries(convs).map(([id, c]) => ({
    id, ...c.contact,
    ...
    <Layout>
      ...
      <div style={{ display:'flex', gap:12, height:'calc(100vh - 190px)', minHeight:400 }}>
        ...
        <div style={{ width:230, background:'#fff', border:'1px solid rgba(0,0,0,.08)', borderRadius:12, overflow:'auto', flexShrink:0 }}>
          {convList.map(c => (
            <div key={c.id} onClick={() => setDestId(c.id)}
              style={{ padding:'11px 13px', cursor:'pointer', borderBottom:'1px solid rgba(0,0,0,.05)', background: destId===c.id ? '#E8F2EB' : 'transparent' }}>
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                <div style={{ width:32, height:32, borderRadius:'50%', background:RC[c.role]||'#6B6560', color:'#fff', display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:700, flexShrink:0 }}>
                  {c.prenom?.[0]||'?'}{c.nom?.[0]||''}
                </div>
          ...
          {newContacts.length > 0 && <>
            <div style={{ padding:'6px 13px', fontSize:10, fontWeight:700, color:'#9E9890', textTransform:'uppercase', marginTop:4, borderTop: convList.length>0?'1px solid rgba(0,0,0,.07)':'none' }}>Nouveau message</div>
            {newContacts.map(c => (
              <div key={c.id} onClick={() => setDestId(c.id)}
                style={{ padding:'9px 13px', cursor:'pointer', borderBottom:'1px solid rgba(0,0,0,.04)', background: destId===c.id ? '#E8F2EB' : 'transparent' }}>
                <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:10, color:'#9E9890' }}>{c.role}</div>
                  </div>
                </div>
              </div>
            ))}
          ...
          {convList.length===0 && newContacts.length===0 && (
            <div style={{ padding:24, textAlign:'center', color:'#9E9890', fontSize:13 }}>Aucun contact disponible.</div>
          )}
        </div>
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', background:'#fff', border:'1px solid rgba(0,0,0,.08)', borderRadius:12, overflow:'hidden' }}>
          {!destId ? (
            <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', color:'#9E9890', flexDirection:'column', gap:8 }}>
              <span style={{ fontSize:36 }}>💬</span>
              <span style={{ fontSize:13 }}>Sélectionnez un contact pour commencer</span>
            </div>
          ) : (
            <>
              {(() => {
                const c = contacts.find(x => x.id===destId) || convs[destId]?.contact
                return c && (
                  <div style={{ padding:'12px 16px', borderBottom:'1px solid rgba(0,0,0,.07)', display:'flex', alignItems:'center', gap:10 }}>
                    <div style={{ width:32, height:32, borderRadius:'50%', background:RC[c.role]||'#6B6560', color:'#fff', display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:700 }}>
                      {c.prenom?.[0]||'?'}{c.nom?.[0]||''}
                    </div>
                    <div>
